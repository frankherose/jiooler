try:
    ...
    os.system("pip install requests user_agent bs4 pycountry uuid")
    import os
    import requests
    from user_agent import generate_user_agent
    from hashlib import md5
...
class Instagram:
    ...
    def CheckInsta(email):
        ...
        if 'email_is_taken' in str(response.text):
            return {'status': 'ok', 'Is_Available': 'true', 'm-anan': '@jiooler'}
        else:
            return {'status': 'bad', 'Is_Available': 'false', 'm-anan': '@jiooler'}
    ...
    def CheckTik(email):
        try:
            ...
            if '"data":{"is_registered":1},"message":"success"' in response.text:
            	...
            	return {'status': 'ok', 'Is_Available': 'false', 'm-anan': '@jiooler'}
            else:
                return {'status': 'bad', 'resposne': 'TurnVPN....!', 'm-anan': '@jiooler'}
        except:
            return {'status': 'Sorry Error Proxy', 'm-anan': '@jiooler'}
...
class BIN:
    ...
    def Process_Bin(P):
        try:
            ...
            return {'status': 'Sorry Error', 'm-anan': '@jiooler'}
...
class Spam:
    ...
    def EmailSpam(email):
        ...
        if 'EMAIL SENT' in response:
            return {'status': 'DONE SEND', 'm-anan': '@jiooler'}
        else:
            return {'status': 'Sorry Error Send', 'm-anan': '@jiooler'}try:
    import os
    import requests
    from user_agent import generate_user_agent
    from hashlib import md5
...
class Spam:
    ...
    def EmailSpam(email):
        ...
        if 'EMAIL SENT' in response:
            ...
            return {'status': 'Sorry Error Send', 'm-anan': '@jiooler'}